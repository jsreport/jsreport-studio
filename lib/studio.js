    ...
    for (let key in documentStore.collections) {
      ...
      if (
        ...
        col.beforeUpdateListeners.insert(0, 'studio-concurrent-save-check', async (q, u, o, req) => {
          ...
          const entities = await col.find(q, req)

          await Promise.all(entities.map(async (currentEntity) => {
            const currentModificationDate = currentEntity.modificationDate

            if (currentModificationDate.getTime() !== lastModificationDate.getTime()) {
              throw reporter.createError(`Entity (_id: ${currentEntity._id}) was modified previously by another source`, {
                status: 400,
