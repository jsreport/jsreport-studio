...
module.exports = (reporter, definition) => {
  ...
  async function replaceThemeVars (content, currentVars) {
    ...
    const replacementVarRegExp = /\$((?!\d)[\w_-][\w\d\['\]_-]*)/g

    // we first replace $var-name syntax to $themeVars['var-name'] to be able
    ...
    const templateSettings = {
      ...
      // disabling evaluate
      evaluate: null,
      // match sass vars syntax
      interpolate: replacementVarRegExp,
      imports: {
        _: null
      }
    }

    const template = _.template(newContent, templateSettings)

    const newCss = await new Promise((resolve, reject) => {
      try {
        resolve(template(currentVars))
      } catch (e) {
        reject(e)
      }
    })
