...
module.exports = (reporter, definition) => {
  ...
  if (definition.options.theme && definition.options.theme.logo != null) {
  ...
  reporter.on('express-configure', () => {
    ...
    app.use((req, res, next) => {
      const redirectPath = '/studio/assets/main.css'

      if (req.path.endsWith(redirectPath)) {
        return next()
      }

      if (req.path.endsWith('main.css')) {
    ...
    app.get('/studio/assets/main.css', (req, res, next) => {
      ...
      readFile(mainCssPath, 'utf8', (err, content) => {
        ...
        const themeVars = Object.assign(
          {},
          DEFAULT_THEME_VARS,
          (definition.options.theme && definition.options.theme.variables != null) ? definition.options.theme.variables : undefined
        )
