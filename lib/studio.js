...
const DEFAULT_THEME_VARS = require('./defaultThemeVars')
...
module.exports = (reporter, definition) => {
  ...
  if (definition.options.logoPath != null) {
    customLogoPath = path.resolve(reporter.options.rootDirectory, definition.options.logoPath)
  ...
  function replaceThemeVars (content, themeVars, cb) {
    if (compiledCss) {
  ...
  reporter.on('express-configure', () => {
    ...
    // hack to make the css work since mini-css-extract-plugin's publicPath option does not work
    app.get('/main.css', (req, res) => {
      res.redirect('/studio/assets/main.css')
    })

    app.get('/studio/assets/main.css', (req, res, next) => {
      ...
      readFile(mainCssPath, 'utf8', (err, content) => {
        ...
        const themeVars = Object.assign({}, DEFAULT_THEME_VARS, definition.options.themeVars)

        replaceThemeVars(content, themeVars, (err, newContent) => {
          if (err) {
            return next(err)
          }

          res.type('css').send(newContent)
        })
      })
    })
    ...
    app.get(`/studio/assets/${extensionsChunkName}`, (req, res) => {
    ...
    app.get('/studio/assets/custom-logo', (req, res, next) => {
      if (!customLogoPath) {
        return res.status(404).end('Custom logo for studio was not configured, use "extensions.studio.logoPath" option if you want to customize it')
      }
